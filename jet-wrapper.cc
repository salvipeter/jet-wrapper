...
#pragma omp parallel for
  for (size_t i = 0; i < n; ++i) {
    ...
    result[i].normal = conv(monge_form.normal_direction());
    result[i].d_min = conv(monge_form.minimal_principal_direction());
    result[i].d_max = conv(monge_form.maximal_principal_direction());
    result[i].k_min = monge_form.principal_curvatures(1);
    result[i].k_max = monge_form.principal_curvatures(0);
  }
  return result;
}
